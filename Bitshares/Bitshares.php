...
function Bitshares(){
	...
	if(empty($modSettings['bts_app_enabled']))
	    fatal_lang_error('bts__app_error1',false);
		
	$subActions = array(
	    'main' => 'bitshares_main',
		'connect' => 'bitshares_connect',
		'auto' => 'bitshares_connectAuto',
		'connectlog' => 'bitshares_connectlog',
		'sync' => 'bitshares_sync',
		'unsync' => 'bitshares_unsync',
		'logsync' => 'bitshares_logsync',
	);
...
function bitshares_Profile(){
	...
	if(isset($_GET['btsdoavatar'])){ // I am not sure how this works, it appears to be separated out from user picture..  I think we may just remove this code
		...
		if(empty($_SESSION['bitshares']['pic']))
...
function bitshares_connectAuto(){
	...
	$regOptions = array(
        ...
        'username' => $_SESSION['bitshares']['name'],
        'email' => $_SESSION['bitshares']['email'],
        'require' => 'nothing',
        'password' => $pass,
		'password_check' => $pass,
...
function bitshares_do_agree(){
	...
	if(isset($_GET['agree'])){
		...
		if(!isset($_POST['accept_agreement']) && !empty($modSettings['requireAgreement']))
			redirectexit('action=bitshares;area=connect;agree');	
    }
}
?>